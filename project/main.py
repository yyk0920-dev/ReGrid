...
#
# 3) UDP 시뮬레이터 모드
#    REGRID_INPUT_MODE=udp REGRID_SIM_UDP_PORT=5000 python3 main.py
#
# UDP GUI sender 예시:
#    python udp_gui_sender.py
#
# -------------------------------------------------------
...
# 추가
from sim_input import get_sim_power_data
...
TEST_FAULT_TYPE = os.getenv(
    "TEST_FAULT_TYPE",
    "OVERLOAD"
)

TEST_VOLTAGE = float(
    os.getenv("TEST_VOLTAGE", "220.0")
)

TEST_CURRENT = float(
    os.getenv("TEST_CURRENT", "1.0")
)
...
def handle_peer_message(message, address, client_socket=None):
    ...
    if msg_type == "FAULT_QUERY":
        ...
        print(
            f"[FAULT-QUERY] "
            f"from={message.get('request_from', address[0])} "
            f"reply_state={reply['state']} "
            f"fault={reply['fault']}"
        )
    ...
    if msg_type == "STATUS":
        ...
        print(
            f"[STATUS] "
            f"peer={message.get('from', address[0])} "
    ...
    print(
        f"[COMM] Unknown peer message "
        f"from {address[0]}: {message}"
    )
...
def get_power_data_for_run():
    ...
    env:
        TEST_VOLTAGE / TEST_CURRENT 사용

    udp:
        PC GUI / Simulink에서 UDP 수신
    """

    # 환경변수 테스트 모드
    if TEST_MODE or INPUT_MODE == "env":
...
def main():
    ...
    if not PEER_NODES:
        ...
        print(
            "[WARNING] "
            "No peer nodes configured. "
            "Fault query will run standalone."
        )

    print(f"[MODE] INPUT_MODE = {INPUT_MODE}")

    if TEST_MODE:

        print(
            f"[TEST] TEST_MODE enabled. "
            f"voltage={TEST_VOLTAGE}V "
            f"current={TEST_CURRENT}A"
        )

    if TEST_FAULT:

        print(
            f"[TEST] TEST_FAULT enabled. "
            f"Forced fault={TEST_FAULT_TYPE}"
        )
    ...
    start_server_thread(
        on_message=handle_peer_message
    )
    ...
    while True:
        ...
        if previous_fault == "NORMAL" and fault != "NORMAL":
            ...
            print(
                f"[FAULT] "
                f"Local fault detected at {NODE_ID}: {fault}"
            )

            peer_replies = query_peer_faults(fault)

            print(
                f"[FAULT] Peer replies: {peer_replies}"
            )
            ...
            if is_single_node_fault(peer_replies):
                ...
                print(
                    "[FAULT] "
                    "Not isolated as single-node fault. "
        ...
        # -------------------------------------------
            ...
            print(
                f"[RECOVERY] "
                f"Local fault cleared at {NODE_ID}. "
                f"Restoring relay."
            )
